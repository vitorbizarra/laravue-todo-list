            ...
            <div class="relative flex h-16 items-center justify-between">
                ...
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                    ...
                    <router-link :to="{ name: 'cadastro' }"
                        :class="[(this.$route.name == 'cadastro') ? 'shadow-md bg-indigo-600 text-white' : 'black hover:shadow', 'rounded-md px-3 text-sm py-2 mr-2 font-medium transition ease-in-out duration-300']"
                        @click="mobileMenuOpen = false">Cadastrar</router-link>
                    <router-link :to="{ name: 'login' }"
                        :class="[(this.$route.name == 'login') ? 'shadow-md bg-indigo-600 text-white' : 'black hover:shadow', 'rounded-md px-3 text-sm py-2 font-medium transition ease-in-out duration-300']"
                        @click="mobileMenuOpen = false">Login</router-link>
                </div>
            </div>
        </div>
